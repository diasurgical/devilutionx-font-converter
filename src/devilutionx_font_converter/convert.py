...
def apply_outer_bevel(
    ...
    """
    Applies an Outer Bevel effect to an image with a transparent background.

    Args:
        image: The input image (must have an alpha channel).
        size: The size of the bevel (corresponds to blur radius).
        angle: The light angle in degrees (0 is up, clockwise).
        highlight_color: The color of the highlight (e.g., '#FFFFFF').
        shadow_color: The color of the shadow (e.g., '#000000').

    Returns:
        PIL.Image.Image: A new image with the bevel effect applied.
    """
    # Generated by Gemini, I don't know if this actually does what it says it does.

    # Get the alpha channel as a grayscale mask and blur it.
    # The blur radius is the 'Size' of the bevel, creating the "Smooth" effect.
    alpha = image.getchannel("A")
    height_map = alpha.filter(PIL.ImageFilter.GaussianBlur(radius=size))

    # Convert alpha to a binary numpy array.
    alpha_np = np.array(alpha) > 0

    # Compute distance from every pixel outside the shape to the nearest edge.
    # This creates the "outer" part of the bevel.
    dist_outside = distance_transform_edt(alpha_np == 0)

    # Clamp the distance to the desired bevel size.
    dist_outside[dist_outside > size] = size

    # Normalize to a 0-255 gradient. We subtract from `size` to make
    # the gradient go from bright (at the edge) to dark (away from the edge).
    gradient_np = (size - dist_outside) * (255 / size)

    # Convert back to a PIL Image to use as our height map.
    height_map = PIL.Image.fromarray(gradient_np.astype(np.uint8))

    # Calculate the light source direction vector.
    angle_rad = math.radians(angle)
    offset_x = round(size * math.sin(angle_rad))
    offset_y = round(-size * math.cos(angle_rad))

    # Create the highlight and shadow masks.
    highlight_source = PIL.ImageChops.offset(height_map, offset_x, offset_y)
    shadow_source = PIL.ImageChops.offset(height_map, -offset_x, -offset_y)

    # The final masks are the difference between the shifted versions and the base.
    highlight_mask = PIL.ImageChops.subtract(highlight_source, height_map)
    shadow_mask = PIL.ImageChops.subtract(shadow_source, height_map)

    # Invert the original alpha to mask out the bevel from the center of the shape.
    inner_hole_mask = PIL.ImageOps.invert(alpha)
    highlight_mask = PIL.ImageChops.darker(highlight_mask, inner_hole_mask)
    shadow_mask = PIL.ImageChops.darker(shadow_mask, inner_hole_mask)

    # The final masks are the difference between the shifted versions and the base.
    highlight_mask = PIL.ImageChops.subtract(highlight_source, height_map)
    shadow_mask = PIL.ImageChops.subtract(shadow_source, height_map)

    # Create solid color layers for highlights and shadows.
    highlight_layer = PIL.Image.new(
        "RGBA", image.size, PIL.ImageColor.getcolor(highlight_color, "RGBA")
    )
    shadow_layer = PIL.Image.new(
        "RGBA", image.size, PIL.ImageColor.getcolor(shadow_color, "RGBA")
    )

    # Composite the final image.
    final_image = PIL.Image.new("RGBA", image.size, (0, 0, 0, 0))
    final_image.paste(shadow_layer, mask=shadow_mask)
    final_image.paste(highlight_layer, mask=highlight_mask)
    final_image.paste(image, mask=image.getchannel("A"))
...
def make_glyphs(
    ...
    if cfg.emboss_size != 0.0:
        img = apply_outer_bevel(
            image=textured_text,
            size=cfg.emboss_size * cfg.supersampling,
            angle=cfg.light_angle,
...
def main_cli() -> None:
    ...
    argparser.add_argument("--emboss_size", type=float, default=0.0)
    argparser.add_argument("--light_angle", type=float, default=36.0)
    argparser.add_argument("--highlight_color", type=str, default="#6a5428")
